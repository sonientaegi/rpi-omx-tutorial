...
typedef struct {
	...
	// Buffer pointer which store REAL data from component. This shall be allocated by client.
	OMX_U8*					pBuffer;
...
int main(void) {
	...
	// Allocate buffer memory which is used by camera.
	print_log("Allocate buffer for camera #71 for output.");
	OMX_INIT_STRUCTURE(portDef);
	portDef.nPortIndex = 71;
	OMX_GetParameter(mContext.pCamera, OMX_IndexParamPortDefinition, &portDef);
	mContext.pBuffer = malloc(portDef.nBufferSize);

	// Allocate buffer to enabled buffer.
	print_log("Allocate buffer header to camera #71 using just allocated buffer.");
	if((err = OMX_UseBuffer(mContext.pCamera, &mContext.pBufferHeader, 71, &mContext, portDef.nBufferSize, mContext.pBuffer)) != OMX_ErrorNone) {
		print_omx_error(err, "FAIL");
		free(mContext.pBuffer);
		mContext.pBuffer = NULL;
		OMX_FreeHandle(mContext.pCamera);
		OMX_Deinit();
		exit(-1);
	}
	print_log("Buffer header is allocated : %d / %d bytes @0x%08x",
	...
	// OMX component only releases instances that allocated by itself.
	// So when using "UseBuffer" client shall release real buffer that client has allocated.
	free(mContext.pBuffer);
	mContext.pBuffer = NULL;
